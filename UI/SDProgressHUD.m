...
- (void)updateIndicators {
    ...
    BOOL isRoundIndicator = [indicator isKindOfClass:[SDUIRoundProgressView class]];
    
    if (mode == SDProgressHUDModeIndeterminate) {
        ...
        [indicator removeFromSuperview];
        self.indicator = [[SDUIBarProgressView alloc] init];
        [self addSubview:indicator];
    }
        ...
        if (!isRoundIndicator) {
            ...
            [indicator removeFromSuperview];
            self.indicator = [[SDUIRoundProgressView alloc] init];
            [self addSubview:indicator];
        }
        if (mode == SDProgressHUDModeAnnularDeterminate) {
            [(SDUIRoundProgressView *)indicator setAnnular:YES];
        }
    }
...
- (void)layoutSubviews {
    ...
    CGSize detailsLabelSize = [detailsLabel.text length] > 0 ?
    [detailsLabel.text boundingRectWithSize:maxSize options:(NSStringDrawingUsesLineFragmentOrigin)
...
@implementation SDUIRoundProgressView
...
- (void)drawRect:(CGRect)rect {
    ...
    if (_annular) {
        // Draw background
        BOOL isPreiOS7 = NSFoundationVersionNumber < kCFCoreFoundationVersionNumber_iOS_8_0;
...
@implementation SDUIBarProgressView
